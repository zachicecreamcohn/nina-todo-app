...
function Titlebar(props) {
    ...
    useEffect(() => {
        ...
        async function fetchFullScreenState() {
          const isFullScreen = await window.electron.invoke('isFullScreen') || false;
    ...
    return (
        ...
        <div className={styles.innerContainer}>
          <div className={styles.left}>
            ...
            />
            <Home size={21} strokeWidth={1} color="white" className={styles.home} />
            {/* <Sear ch /> FEATURE IN PROGRESS */}
          </div>
          <div className={styles.center}></div>
          <div className={styles.right}>
            <Plus size={23} strokeWidth={1} color="white" className={styles.plus} />
            <Settings size={21} strokeWidth={1} color="white" className={styles.settings} />
          </div>
          </div>
        </div>
      );
